            ...
            <span className='w-fit h-fit flex flex-wrap items-center justify-center gap-9'>
                {isLoading ? (
                    Array(3).fill(0).map((_, i) => (
                        <div key={i} className="w-[140px] h-[190px] bg-gray-200 animate-pulse rounded" />
                    ))
                ) : (
                    books.map((book, i) => (
                        <motion.div key={i}
                                    ...
                                    whileInView='visible'
                                    custom={i}
                                    className="relative overflow-hidden w-[140px] h-[190px]"
                        >
                            <OptimizedImage src={book.imageUrl} alt='book' fill={true} className='cursor-pointer hover-image-filter duration-300 object-cover'/>
                        </motion.div>
                    ))
                )}
            </span>
