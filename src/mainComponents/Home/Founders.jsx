            ...
            <div className='w-full flex items-center justify-evenly text-start flex-wrap gap-5'>
                {founder.map((person, i) => (
                    <span key={i} className='w-[240px] h-[320px] p-2 gap-2 flex flex-col items-start justify-start'>
                        <motion.div className='w-full h-[70%] flex items-center justify-center bg-[#555] rounded-md overflow-hidden relative'
                            variants={scaleBig}
                            initial='hidden'
                            whileInView='visible'
                            transition={{
                        ...
                        >
                          <OptimizedImage src={person.imageUrl} alt='person' fill={true} className='object-cover' />
                        </motion.div>
