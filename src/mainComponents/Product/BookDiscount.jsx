  ...
  return (
    <section className="w-full min-h-screen flex flex-col items-center justify-between ">
        <div className='sm:w-[85%] 2md:w-[80%] w-[90%] min-h-fit flex flex-col lg:flex-row items-start justify-center mt-[5vh] 2md:pt-[0] pt-[15vh] gap-[30px] relative'>
            <motion.span className='lg:w-[320px] w-full  sm:h-[350px] h-[300px] bg-white/50 flex flex-col items-center justify-start text-[--blackColor] relative overflow-hidden'
                    variants={appear}
                    initial='hidden'
                    whileInView='visible'
                    transition={{delay: 0.6, duration: 0.4}}
            >
                <OptimizedImage src='https://i.pinimg.com/564x/02/ec/57/02ec57235e5f6cefda6b02064db7d522.jpg' alt='comic book' fill={true} className='object-cover image-filter' />
            ...
            <div className='lg:w-[calc(100%-350px)] w-full min-h-fit flex flex-wrap items-start lg:justify-start justify-center gap-4'>
                    {isLoading ? (
                        Array(3).fill(0).map((_, i) => (
                            <ComicCardSkeleton key={i} variant="vertical" />
                        ))
                    ) : (
                        comic.map((book, i) => (
                            <motion.div key={i} className='w-[220px] h-[350px] flex flex-col items-start justify-start bg-[--whiteColor] text-[--blackColor] p-2 cursor-pointer'
                                ...
                                custom={i}
                            >
                                <span className='w-full h-[60%] flex items-center justify-center bg-[#555] overflow-hidden relative'>
                                    <OptimizedImage src={book.imageUrl} alt='comic book' fill={true} className='object-cover' />
                                </span>
